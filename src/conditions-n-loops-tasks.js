...
function isIsoscelesTriangle(a, b, c) {
  if (a <= 0 || b <= 0 || c <= 0 || a + b < c || a + c < b || b + c < a)
    return false;
  if (a + b === a + c || b + a === b + c || c + a === c + b) return true;
  return false;
}
...
function convertToRomanNumerals(num) {
  ...
  if (num === 10) return dictionary.get(10);
  let out = '';
  const units = num % 10;
  const dozens = num - units;
  for (let i = 0; i < dozens; i += 10) {
    out += dictionary.get(10);
  }
  return out + dictionary.get(units);
}
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 1; i < proposal.length; i += 1) {
    out += proposal[i];
  }
  return out;
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length - 1; i += 1) {
    ...
    for (let j = 0; j < arr.length; j += 1) {
      if (j < i) sumLeftEl += arr[j];

      if (j > i) sumRightEl += arr[j];
    }
    if (sumLeftEl === sumRightEl) return i;
  }
...
function getSpiralMatrix(size) {
  ...
  function innocent() {
    ...
    for (let i = size - step - 1; i >= step + 1; i -= 1) {
      arr[i][step] = c;
      c += 1;
    }
    ...
    if (step <= Math.ceil(size / 2)) innocent();
    return arr;
  }
  return innocent();
}
